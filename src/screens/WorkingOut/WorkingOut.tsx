...
export function WorkingOut({ navigation, route }: NavigationPageProps) {
  ...
  const [currentWorkout, setCurrentWorkout] = useState<WorkoutDetails | null>(
    null,
  );
  ...
  const [currentExercise, setCurrentExercise] =
    useState<WorkoutExercisesDetails | null>(null);
  ...
  const handleGetHistory = useCallback(async () => {
    if (!workoutId) return; // Type Safety
    const lastWorkout = (
      await workoutDatabase.getWorkoutHistory(workoutId)
    ).sort((a, b) => {
      ...
      >
        <ScrollView style={styles.container}>
          <Text style={styles.title}>
            Exercise:{" "}
            {currentWorkout?.exercises[currentExerciseIndex].exercise_name}
          </Text>
          ...
          <View style={styles.repInputContainer}>
            {currentExercise?.sets?.map((set, index) => {
              return (
                <View key={index}>
                  <Text>
                    Set {set.set_number} - {set.repetitions} Reps
                  </Text>
                  <View style={styles.setsContainer}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => {
                        const currentEditedSet = savedSets.filter(
                          (editedSet) =>
                            editedSet.setNumber === String(set.set_number),
                        );
                        const updatedSets = savedSets.filter(
                          (s) => s.setNumber !== String(set.set_number),
                        );

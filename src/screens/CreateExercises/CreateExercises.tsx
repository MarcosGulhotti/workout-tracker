...
export function CreateExercises({ navigation, route }: NavigationPageProps) {
  ...
  const [savedExercises, setSavedExercises] = useState<
    { exerciseName: string; sets: ExerciseSet[] }[]
  >([]);

  const selectedWorkout = useMemo(
    () => (route.params ? route.params.selectedWorkout : null),
    [route],
  );
  ...
  const handleSaveExercises = async () => {
    ...
    savedExercises.forEach(async (elm) => {
      const exercises = workoutDatabase.addExerciseToWorkout(
        selectedWorkout.id,
        elm.exerciseName,
        elm.sets,
      );
                  ...
                  {exercise.sets.map((set, index) => (
                    <View style={styles.setsTextContainer}>
                      <Text
                        ...
                        key={index}
                      >{`Set ${set.set_number}: ${set.repetitions} reps`}</Text>
                    </View>
                  ))}
                </View>
              </View>

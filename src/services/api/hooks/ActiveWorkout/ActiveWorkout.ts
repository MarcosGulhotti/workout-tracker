...
export const saveCompletedWorkout = async ({ workout_id, workoutName, date, savedWeights }: CompletedWorkout) => { //! TODO Refactor
    return new Promise<void>((resolve, reject) => {
        ...
        executeSql({
            sql: 'INSERT INTO completed_workouts (workout_id, workout_name, date) VALUES (?, ?, ?);',
            args: [workout_id, workoutName, date],
            successCallback: (_, { insertId: completedWorkoutId }) => {
                ...
                savedWeights.forEach(exercise => {
                    executeSql(
                        {
                            sql: 'INSERT INTO completed_exercises (completed_workout_id, exercise_name) VALUES (?, ?);',
                            args: [workout_id, exercise.exerciseName], // Use o nome do exercício ou uma referência que você possui
                            successCallback: (_, result) => {
                                ...
                                exercise.sets.forEach(set => {
                                    executeSql(
                                        {
                                            sql: 'INSERT INTO completed_sets (completed_exercise_id, set_number, repetitions, weight) VALUES (?, ?, ?, ?);',
                                            args: [completedExerciseId, set.setNumber, set.repetitions, set.weight],
                                            successCallback: () => console.log(`Set ${set.setNumber} do exercício ${exercise.exerciseId} salvo.`),
        ...
        executeSql(
            {
                ...
                successCallback: (_, { rows }) => {
                    ...
                    executeSql({
                            sql: `SELECT completed_exercise_id, set_number, weight
                                  FROM completed_sets
                                  WHERE completed_exercise_id = ?
                                  ORDER BY set_number ASC;`,
                            args: [completedExercise.exerciseId],
                            successCallback: (_, { rows }) => {

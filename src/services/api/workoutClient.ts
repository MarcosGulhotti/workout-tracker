...
import {
  CreateCardioTable,
  CreateCompletedExercisesTable,
  CreateCompletedSetsTable,
  CreateCompletedWorkoutsTable,
...
export function addExerciseToWorkout(
  ...
  executeSql(
    ...
    [newExerciseId, workout_id, exercise_name],
    async (_, result) => {
...
const getExercisesForWorkout = async (
  workoutId: string,
): Promise<Exercise[]> => {
...
export const getCompletedWorkoutDetails = async (
    ...
    executeSql(
      ...
      (_, { rows }) => {
        ...
        executeSql(
          ...
          [completedWorkoutId],
          (_, { rows }) => {
            const exercises: CompletedExercise[] = rows._array.map(
              (exercise) => ({
              ...
              (_, { rows }) => {
                rows._array.forEach((set) => {
                  const exercise = exercises.find(
                    (e) =>
                      e.exerciseId === set.completed_exercise_id.toString(),
                  );
                  if (exercise) {
                    exercise.sets.push({
